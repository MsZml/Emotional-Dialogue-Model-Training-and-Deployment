...
"""
# 初始化模型
client = ZhipuAI(api_key="238333f13fdb4e5ff39d7.GwngHUrulaFNjfgw")  # 替换为你的API Key
#加载Embeddingmodel
...
def generate_style_data(style_name, num_samples=50):
    ...
    #从本地文件加载用户输入
    user_inputs = []
    with open('cleaned_output.txt',encoding='utf-8') as f:
        for line in f:
            # 直接读取每行内容并去除换行符
    ...
    if not user_inputs:
        raise ValueError("文件内容为空或未成功加载数据，请检查："
                         "1.文件路径是否正确 2.文件是否包含有效字符")

    #初始化顺序索引
    current_index = 0  #添加索引技术器
